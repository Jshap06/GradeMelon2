...
export default function Grades({
	...
	console.log(period);
	console.log("up there douchebag")
	useEffect(() => {
	...
	useEffect(() => {
		try {
			if (!grades && client) {
				//setLoading(true);
				try {
					client.getparseGrades().then((res) => {
						console.log(res);
						setGrades(res);
						setPeriod(res.period.index);
						setLoading(false);
					});
				} catch (err) {
					console.log(err);
...
//hey
	...
	},[client,grades,grades2])


	const update = (p: number) => {
		console.log(p);
		console.log("por favor please fucking god just make sense")
		console.log(period)
		setLoading(true);
		try {
			client.getparseGrades(grades.periods[p].gu).then((res) => {
				console.log("wwfwfawwhatokaysure")
				console.log(res);
				setGrades(res);
				setPeriod(p);
			...
			console.log("i am indescribily confused right now")
			console.log(err);
			createError(err.message);
		};
	};
	...
	return (
		<motion.div className="p-5 md:p-10 flex-1 overflow-hidden">
			...
			{loading ? (
				...
				<div className="p-1 mx-auto md:mx-0">
