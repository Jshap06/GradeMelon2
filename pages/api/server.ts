...
async function getStudentPhoto(details:details){
    console.log(details)
...
async function getHomePageGrades(details) {
    ...
    const headers = {
        ...
        await axios.get(details.domain+"/PXP2_GradeBook.aspx?AGU=0")
...
async function getAssignments(details){
    return new Promise(async (res, rej)=>{
        try{
        await axios.post(details.url2,details.senddata,{headers:details.headers2})
        var response2= await this.session.post(details.url, details.data,{headers:details.headers});
        var response3= await this.session.post(details.url3,details.data3,{headers:details.headers})}
...
export default async function handler(req, res) {
    ...
    res.json({status:false,message:"u really thought you ate?"})}
